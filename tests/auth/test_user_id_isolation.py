...
bucket — bypassing per-user spend limits, granting free tokens into a shared
pool, and leaking cross-studio spend/balance data.

These tests prove the guard holds across every write path (can_spend,
try_spend, record_spend, grant_tokens) and the one read path that bypassed it
(gamer_balance). To verify they catch the regression:
  1. Remove `_validate_user_id(user_id)` from any of manager.can_spend /
     try_spend / record_spend / grant_tokens, or from routes.gamer_balance.
  2. Re-run — the corresponding "blocked" tests will fail (ValueError no
     longer raised / HTTP 400 no longer returned), confirming the guard is
     load-bearing.
...
class TestGrantTokensBlocksPlaceholders:
    """grant_tokens() must raise ValueError for sentinel user_ids.

    Reachable via the S2S /api/credits/grant endpoint with just the shared
    service key (used by standalone studios) — a bug in any studio's own
    user_id resolution could otherwise mint free tokens into a shared bucket.
    """

    @pytest.mark.parametrize("bad_id", ["default", "", "anonymous"])
    def test_raises_for_sentinel_user_id(self, db, bad_id):
        from core.credits.manager import grant_tokens

        with pytest.raises(ValueError, match="reserved placeholder"):
            db.run_until_complete(grant_tokens(bad_id, 50, "test_grant"))
...
class TestS2SEndpointsBlockPlaceholders:
    ...
    def test_grant_endpoint_rejects_default(self, client):
        resp = client.post(
            "/api/credits/grant",
            json={"user_id": "default", "amount": 50, "reason": "test"},
            headers={"X-Service-Key": _VALID_KEY},
        )
        assert resp.status_code == 400, resp.text
        assert "reserved placeholder" in resp.json()["detail"]
...
# ── 2b. Gamer/Overlay endpoint: reads must not disclose the placeholder bucket ──

class TestGamerBalanceBlocksPlaceholders:
    """GET /api/gamer/tokens/balance took X-User-Id straight to get_balance()
    with no placeholder check — any S2S-authenticated caller (leaked service
    key, or a caller that itself fell back to a sentinel) could read whatever
    ended up in the shared "default" bucket. Added alongside the write-path
    guard so read and write paths are covered the same way.
    """

    def test_gamer_balance_rejects_default(self, client):
        resp = client.get(
            "/api/gamer/tokens/balance",
            headers={"X-User-Id": "default", "X-Service-Key": _VALID_KEY},
        )
        assert resp.status_code == 400, resp.text
        assert "reserved placeholder" in resp.json()["detail"]

    def test_gamer_balance_allows_real_user_id(self, client):
        resp = client.get(
            "/api/gamer/tokens/balance",
            headers={"X-User-Id": "u1", "X-Service-Key": _VALID_KEY},
        )
        assert resp.status_code == 200, resp.text
...
class TestReintroductionCanary:
    ...
    def test_grant_tokens_calls_validate(self, db):
        """Confirm grant_tokens calls _validate_user_id."""
        from core.credits import manager
        ...
        try:
            try:
                db.run_until_complete(manager.grant_tokens("u1", 5, "canary"))
            except Exception:
                pass
        finally:
            manager._validate_user_id = original
        ...
        assert called_with == ["u1"], (
            "grant_tokens() no longer calls _validate_user_id — "
            "S2S callers can mint free tokens into a placeholder bucket"
        )
